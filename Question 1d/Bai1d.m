...
M=5;% number of iteration when refine mesh
...
for jj=1:M
    ...
    x=zeros(N+1,1);
    for i_iter=1:N+1
        x(i_iter)=1-cos(((i_iter-1)*pi)/(2*N));
%         x(i_iter)=(i_iter/N)^2;
    end
    ...
    % Creare the Matrix and create vector b
    A=zeros(N,N);
    b=zeros(N,1);
    for i_iter=1:N
        ...
        if(i_iter==1)
            ...
            b(i_iter)=(f(x(i_iter))+f(x(i_iter+1)))/2.0-aa*a1;
        else
            if(i_iter==N)
                A(i_iter,i_iter-1)=a1;
                A(i_iter,i_iter)=-(a1+b1);
                b(i_iter)=(f(x(i_iter))+f(x(i_iter+1)))/2.0-bb*b1; 
            else
                A(i_iter,i_iter-1)=a1;
                A(i_iter,i_iter+1)=b1;
                A(i_iter,i_iter)=-(a1+b1);
